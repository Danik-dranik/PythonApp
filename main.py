...
class SigninScreen(Screen):
    ...
    def submit(self, *args):
        ...
        if re.search(regex, user_email) and user_namee != "" and user_password != "" and user_nnum_list != "" and user_type_user != "":
            try:
                print("connection")
                cursor.execute("SELECT * FROM users WHERE mail = ? AND password = ?", (user_email,user_password,))
                cur = cursor.fetchone()
                if cur is None:
                    ...
                    val = (user_email, user_namee, user_password, user_nnum_list, user_type_user)
...
class CallBackScreen(Screen):
    ...
    def sendEmail(self, *args):
        ...
        print("login success")
        new_mesage =message+ " " + user_email_data[0]
        server.sendmail(sender_email, rec_email, new_mesage)
...
class MainApp(MDApp):
    ...
    def on_start(self):
        ...
        screens = [MenuScreen(name="menu"),LoginScreen(name="login"), SigninScreen(name="signin"), AddPlaceScreen(name="addplace")]
